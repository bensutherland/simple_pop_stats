...
plot_summarize_100_sim <- function(axis_label="repunit"){
    ...
    # Only do the following if your repunits file has a "region" column
    ...
    png(file = "03_results/summarize_100_sim_plot.png",width=969)
    ...
    if("region" %in% colnames(repunits)){
          ...
          yy <- barplot(as.matrix(t(subset(plot_temp2, select=c("coll_post_mean_pi",
                        ...
                        ,xlab = "", ylab = "",horiz=TRUE,col=c("white","black","grey")
          ...
          yy <- barplot(as.matrix(t(subset(plot_temp2, select=c("coll_post_mean_pi",
                                                                "repunit_post_mean_pi_extra"))))
                        ,xlab = "", ylab = "",horiz=TRUE,col=c("white","black")
                        ,axes = FALSE, las=1,xlim=c(0,1),cex.names=0.75)
