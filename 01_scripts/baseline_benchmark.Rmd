...
```{r,echo=FALSE}
folder <- "Coastwide_V1.0.0_2020-10-29"
species <- "steelhead"
datatype <- "microsat"
```
...
```{r,echo=FALSE, message=FALSE, comment=NA}
tray.fn <- list.files(paste0(getwd(),"/Baseline_summary/"),pattern="*highest_tray_number.txt$",full.names = T)
library(readr)
if (length(tray.fn)==1){
...
# example for how to format the reference. TBD to make it automated.
#cat(str_break("Population structure of eulachon Thaleichthys pacificus from Northern California to Alaska using single nucleotide polymorphisms from direct amplicon sequencing (2020). Ben J. G. Sutherland, John R. Candy, Kayla Mohns, Olivia Cornies, Kim Jonsen, Khai Le, Richard G Gustafson, Krista M. Nichols, Terry D. Beacham. Canadian Journal of Fisheries and Aquatic Sciences, https://doi.org/10.1139/cjfas-2020-0200."), sep = '\n')
#cat(str_break("Accurate estimation of conservation unit contribution to coho salmon mixed-stock fisheries in British Columbia, Canada, using direct DNA sequencing for single nucleotide polymorphisms (2020). Terry D. Beacham, Colin Wallace, Kim Jonsen, Brenda McIntosh, John R. Candy, Eric B. Rondeau, Jean-Sébastien Moore, Louis Bernatchez, Ruth E. Withler. Canadian Journal of Fisheries and Aquatic Sciences, https://doi.org/10.1139/cjfas-2019-0339."), sep = '\n')

library(dplyr)
tmp_species <- species
tmp_datatype <- datatype
cite <- filter(citations,species %in% tmp_species)
cite <- filter(citations,datatype %in% tmp_datatype)
if(length(cite>0)){
