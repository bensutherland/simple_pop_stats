...
```{r,echo=FALSE, comment=NA}
fn.6 <- list.files(paste0(getwd(),"/100_sims/"),pattern="*.pdf$",full.names = T,ignore.case=TRUE)
if(length(fn.6>0)){
str_break(fn.6)
...
\includepdf[scale=0.75,pages={-},pagecommand={\subsection{Dendrogram}\captionof{figure}{Dendrogram of collections included in the benchmarked baseline}}]{"`r  list.files(paste0(getwd(),"/Dendrogram/"),pattern="*.pdf$",full.names = T,ignore.case=TRUE)`"}

\newpage
<!-- Files are produced by full_sim() and plot_summarize_100_sim()  -->
\includepdf[scale=0.75,pages={-},pagecommand={\subsection{100\% Simulation summary}\captionof{figure}{Mean accuracy to collection (white), repunit (grey), or region (if applicable - dark grey)}}]{"`r  list.files(paste0(getwd(),"/100_sims/"),pattern="*.pdf$",full.names = T,ignore.case=TRUE)`"}
\newpage
