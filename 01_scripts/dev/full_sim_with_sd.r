...
full_sim <- function(rubias_base.FN = "C:/Users/RondeauE/Documents/TEMP/bch_122296_ind_367_pop_389_amp_Rubias_2020-10-07_GSI-base.txt.gz"
...
#  rubias_base <- filter(rubias_base,!(collection %in% c(NULL)))
  ...
  coll_to_rep_filt_all_2 <- all_collection_results %>%
                                group_by(collection_scenario, repunit, iter) %>%
                                summarise_at(.vars=c("post_mean_pi","mle_pi","true_pi")
                                            , .funs=c(sum="sum")) %>%
                                group_by(collection_scenario, repunit) %>%
                                 summarise_at(.vars=c("post_mean_pi_sum","mle_pi_sum","true_pi_sum")
                                  , .funs=c(mean="mean",sd="sd"))
  # Provide names
  ...
  # Provide names
  names(coll_to_rep_filt_all_2) <- c("collection_scenario"
                                   ,"repunit"
                                   ,"repunit_post_mean_pi"
                                   ,"repunit_mle_pi"
                                   ,"repunit_true_pi"
                                   ,"repunit_post_mean_sd"
                                   ,"repunit_mle_sd"
                                   ,"repunit_true_sd")
  ...
  write_tsv(x = coll_to_rep_filt_all_2, path = paste0(result.path, "collection_100_stats_all_reps_sd_",format(Sys.time(), "%Y-%m-%d"),".txt"))
